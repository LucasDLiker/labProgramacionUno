...
int agregarCliente(eCliente list[], int len)
{
    ...
    else
    {
        ...
        getValidAlfaNumerico(" Ingrese el domicilio: ", cliente.domicilio, 8, 50);
...
int modificarCliente(eCliente list[], int len)
{
    ...
    getValidInt(" Ingrese el ID del cliente que desea modificar: ",&idCliente,1,200);

    found=buscarClientePorID(list,len,idCliente);

    if(found>= 0 && list[found].isEmpty == 1)
    {
        mostrarCliente(list[found]);

        getValidString(" Desea modificar este cliente? s/n --> ",&respuesta,1,1);
        ...
        if(respuesta=='s')
        {
            do
            {
                ...
                {
                case 1:
                    getValidString("\n Ingrese nuevo apellido: ",list[found].apellido,3,50);
                    break;
                case 2:
                    getValidString("\n Ingrese nuevo nombre: ",list[found].nombre,3,50);
                    break;
                case 3:
                    getValidSexo("\n Ingrese nuevo sexo: ",&list[found].sexo);
                    break;
                case 4:
                    getValidAlfaNumerico("\n Ingrese nueva direccion: ", list[found].domicilio, 8, 50);
                    break;
                case 5:
                    printf("\n Modifacaciones realizadas...\n\n");
                    break;
                default:
                    printf("\n Opcion incorrecta. Ingrese una opcion del 1 al 5.\n\n");
                    system("pause");
                    break;
                }
            }
